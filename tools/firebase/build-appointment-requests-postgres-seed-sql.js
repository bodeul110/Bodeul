...
function main() {
  ...
  const sql = buildAppointmentSeedSql(plan, {rollback: options.rollback});
...
function resolveOutputPath(outputPath, rollback) {
  ...
  return path.resolve(
      process.cwd(),
      "reports",
      `appointment-requests-postgres-${suffix}-${formatTimestamp(new Date())}.sql`,
  );
}

function printValidationSummary(plan) {
  console.log("예약 요청 PostgreSQL seed 검증 결과");
  console.log(`- 상태: ${plan.status}`);
  console.log(`- 예약 요청: ${plan.rowCount}건`);
...
function printHelp() {
  console.log("Firestore 예약 요청 PostgreSQL seed SQL 생성기");
  console.log("");
  console.log("사용법:");
  console.log("  node build-appointment-requests-postgres-seed-sql.js --file backups/firestore-backup.json --check");
  console.log("  node build-appointment-requests-postgres-seed-sql.js --file backups/firestore-backup.json");
  console.log("  node build-appointment-requests-postgres-seed-sql.js --file backups/firestore-backup.json --rollback");
}
...
try {
  ...
  console.error("예약 요청 PostgreSQL seed 처리 중 오류가 발생했습니다.");
