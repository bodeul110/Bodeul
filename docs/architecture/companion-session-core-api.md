...
현재 MVP 규모에서도 예약과 동행 상태를 서로 다른 저장소에서 수정하면 매칭 이후 취소, 세션 완료와 예약 완료를 원자적으로 처리할 수 없다. 반면 모든 실시간 데이터를 한 번에 옮기면 검증 범위가 너무 커진다. 그래서 낮은 빈도의 상태·메모·리포트를 먼저 관계형 트랜잭션 경계로 옮기고, 채팅과 legacy 위치는 이후 #221에서 보관·파기 정책과 함께 전환했다. 현재 legacy 위치는 기본 OFF이며 환자 위치 목표를 별도로 적용한다.
...
- 브라우저와 Android의 `anon`, `authenticated`, `service_role`에는 업무 table 권한을 부여하지 않는다. 관리자 runtime은 이 실시간 업무 테이블에 직접 쓰지 않는다. 배정·결제 등 관리자 업무는 별도로 허용한 제한 함수만 실행한다.
