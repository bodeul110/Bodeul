...
| 후보 | 장점 | 리스크 | 현재 판단 |
| --- | --- | --- | --- |
| Cloud Firestore | Auth/Rules/Functions/FCM/Storage와 통합이 쉽고, 문서 단위 실시간 구독이 자연스럽다. 서버 없이 Android 앱과 관리자 웹이 같은 계약을 공유할 수 있다. | 조인, 복잡한 집계, 장기 분석 쿼리에는 약하다. 읽기/쓰기 단위 과금이라 실시간 리스너와 관리자 대시보드 스캔을 관리해야 한다. | 현재 MVP 기본 선택지다. |
| MySQL/PostgreSQL + Spring/Node API | 관계형 무결성, 조인, 정산/통계 쿼리에 강하다. | API 서버, 인증/권한, 배포, 장애 대응, 스케일링을 별도로 운영해야 한다. 초기 화면과 운영 도구 구현 속도가 느려진다. | 정산/통계 요구가 커질 때 재검토한다. |
| Supabase/PostgreSQL | PostgreSQL 기반이며 인증, 스토리지, Realtime을 같이 제공한다. SQL과 RLS를 쓸 수 있다. | 현재 앱은 Firebase Auth, FCM, Storage, Functions에 이미 강하게 결합돼 있다. 이중 인증/권한 체계를 만들 가능성이 높다. | 신규 전환 이익보다 이관 비용이 크다. |
