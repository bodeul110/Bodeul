...
Supabase 개발 DB 준비 이후, 관리자 웹과 Android 앱이 PostgreSQL을 언제, 어떤 경로로 사용해야 하는지 1차 경계를 정한다. 2026-07-02 현재 이 경계는 `bodeul-api` 코드로 일부 구현됐다.
...
| API 배포 환경 | 미정 | Oracle VM 또는 동등 실행 환경 결정 필요 |
...
`bodeul-dev-api-01` 또는 동등 실행 환경은 아래 조건을 만족할 때 만든다.
...
2026-07-02 현재 서버 골격과 첫 read API는 준비됐지만, 실제 클라이언트 연결과 배포 환경은 아직 결정되지 않았다.
...
- [Issue #122](https://github.com/bodeul110/Bodeul/issues/122)는 관리자 웹 API 환경변수와 CORS origin 설정을 추적한다.
...
- `uuid` 전이 취약점 검토는 [Issue #103](https://github.com/bodeul110/Bodeul/issues/103)에서 별도 추적한다.
