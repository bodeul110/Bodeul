...
Supabase 개발 DB 준비 이후, 관리자 웹과 Android 앱이 PostgreSQL을 언제, 어떤 경로로 사용해야 하는지 1차 경계를 정한다. 2026-07-07 현재 이 경계는 `bodeul-api` 코드, 관리자 웹 API 모드, 로컬 비교 도구로 일부 구현됐다.
...
| API 배포 환경 | preview 검증 준비 | #140에서 Oracle API 실행 환경을 사용해 실연동 검증을 진행한다. production 실행 환경은 #134 이후 별도 확정 |
...
2026-07-07 현재 서버 골격, 첫 read API, 관리자 웹 API 모드 화면, 로컬 비교 도구는 준비됐다. 남은 검증은 Oracle preview API URL, Supabase 개발 DB 연결, Firebase Admin 인증, Vercel 또는 Firebase Hosting preview origin CORS, 실제 `/admin/hospital-guides?limit=50` 응답 비교다.
...
- [Issue #123](https://github.com/bodeul110/Bodeul/issues/123)는 병원 가이드 Firestore/API 응답 비교 기록을 추적한다.
- [Issue #134](https://github.com/bodeul110/Bodeul/issues/134)는 관리자 웹 production 배포 기준을 확정한다.
- [Issue #140](https://github.com/bodeul110/Bodeul/issues/140)은 Oracle/Vercel 기반 preview 실연동 검증 환경을 구축해 #123의 blocker를 해소한다.
