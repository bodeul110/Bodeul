...
FCM 기기 토큰은 Firebase 인증·메시징 보조 데이터로 Firestore `users` 문서에 유지한다. Core API는 PostgreSQL 커밋 이후 토큰을 읽어 고정 안내만 발송하며 채팅 본문과 좌표는 FCM이나 운영 로그에 넣지 않는다. 기존 Firebase Storage 채팅 첨부는 Storage Rules가 legacy Firestore 세션 ID로 참여자를 확인하므로 기존 앱과 세션만 이 경로를 유지한다. Firestore 보조 문서가 없는 Core-only 세션은 Android가 원본을 Core API에 전송하고, Core API가 PostgreSQL 참여 관계와 만료 상태를 확인해 전용 Firebase Storage 버킷에 저장·조회한다. 클라이언트의 해당 경로 직접 쓰기는 계속 거부한다.
