...
- Core-only 세션 채팅 첨부는 Spring Core API 서버 중계 계약으로 전환했다. 개발·production Firebase 기본 버킷에는 각 Cloud Run 런타임 계정의 버킷 단위 `roles/storage.objectUser`만 부여했고 Preview 배포를 완료했다. 인증된 실기기 업로드·다운로드와 production 적용은 남아 있다.
...
- production PostgreSQL V13 dump를 격리 PostgreSQL 17에 복원해 schema, row 수, owner, ACL, RLS, 인덱스와 제약 일치를 검증했다.
