...
- DDL과 migration은 한 곳에서만 관리한다. 초기 migration 소유자는 `bodeul-core-api`로 둔다.
- 런타임 role은 `core`, `admin`, `migration` 용도로 분리하고 최소 권한을 적용한다.
...
Supabase Data API를 클라이언트에서 사용하지 않는 테이블은 공개 API 노출 대상에서 제외한다. 기존 `public` 테이블은 grant와 RLS 상태를 점검하고, 새 서버 전용 테이블은 비공개 schema 사용을 우선 검토한다.

## 연결 방식

| 실행 환경 | PostgreSQL 연결 기준 |
| --- | --- |
| Vercel Next.js | 짧은 수명의 서버리스 연결이므로 Supavisor transaction pooler를 사용한다. prepared statement 비활성화 여부를 DB client 기준으로 확인한다. |
| OCI Spring Core API | 장기 실행 서버다. OCI에서 IPv6 direct 연결이 가능하면 direct connection을 사용하고, IPv4 제약이 있으면 Supavisor session mode를 사용한다. |
| migration, backup, restore | 가능하면 direct connection을 사용하고 런타임 role과 분리한 migration role을 사용한다. |

두 서버의 application pool 크기는 Supabase connection limit을 기준으로 작게 시작한다. 연결 문자열은 Vercel과 OCI의 비공개 환경 설정에만 둔다.
...
1. Spring Core API 저장소, Java LTS, Spring Boot 기준과 CI를 만든다.
