...
이 문서는 멘토링에서 합의한 운영 목표를 기록한다. Spring Core API의 개발 실행 환경은 이 구조로 전환됐지만, 관리자 Next.js 전환과 클라이언트 연결, production 환경은 아직 완료되지 않았다.
...
| 현재 자산 | 처리 방향 |
| --- | --- |
| `api/` Node.js 서버 | 운영 후보에서 제외하고 동결된 계약 검증용 프로토타입으로 보존한다. 관리자 Next.js가 병원 가이드 계약을 대체한 뒤 소스와 CI를 함께 제거한다. |
...
5. Kakao Local REST proxy와 Android Core API client 구현을 완료했다. Cloud Run secret 주입과 실호출을 검증한다.
6. 관리자 웹을 Next.js로 단계 이전하고 관리자 서버가 PostgreSQL에 직접 접근하게 한다.
7. 관리자 Next.js가 병원 가이드 계약을 대체한 뒤 Node `api/`를 제거하고, 기존 Firestore 직접 접근을 도메인별로 종료한다.
