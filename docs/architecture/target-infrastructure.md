...
- 개발 DB의 채팅·위치는 PostgreSQL V8·V9 schema, 최소 권한과 보관 계약을 적용했다. Core API snapshot·메시지·읽음·위치 계약과 V10 DB Broadcast는 구현했으며, 개발 DB V10·private 채널 RLS·Android 전환 전까지 Firestore legacy 쓰기를 제한적으로 유지한다.
