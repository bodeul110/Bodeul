...
- Core-only 세션 채팅 첨부는 Spring Core API 서버 중계 계약으로 전환했다. 개발·production Firebase 기본 버킷에는 각 Cloud Run 런타임 계정의 버킷 단위 `roles/storage.objectUser`만 부여했으며, Preview 배포와 인증된 실기기 종단 검증은 남아 있다.
