...
- Oracle Free Tier 계정 잠금 이후 Spring Core API의 개발 실행 환경은 Google Cloud Run으로 변경했다.
...
4. Cloud Run에 Spring preview를 배포하고 HTTPS, revision, 로그, 헬스체크와 실제 Firebase token을 검증한다.
