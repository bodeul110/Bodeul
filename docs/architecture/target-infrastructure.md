...
- 개발 DB의 채팅·위치는 PostgreSQL V8~V10 schema·trigger, 최소 권한과 보관 계약을 적용했다. Core API snapshot·메시지·읽음·위치 계약과 V11 privileged Broadcast publisher는 구현했으며, 개발 DB publisher·V11·private 채널 RLS·Android 검증 전까지 Firestore legacy 쓰기를 제한적으로 유지한다.
