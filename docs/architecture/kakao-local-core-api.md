...
- Android의 기존 직접 호출 코드는 첫 실기기 검증 전까지 실패 시 rollback 경로로만 유지한다.
...
- Android rollback 직접 호출은 검증 기간에만 유지한다. Core API 성공률과 fallback 동작을 실기기에서 확인한 뒤 `kakaoRestApiKey` 리소스와 직접 호출 코드를 제거한다.
