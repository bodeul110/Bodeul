...
결제 원장은 PostgreSQL과 Core API 응답이다. `appointment_requests.payment_status_code`는 목록·매칭용 현재 상태 projection이며, 1:1 `appointment_bank_transfer_payments`가 예상 금액·입금자명·선택 기한·실입금액·확인·환불 정보를 보관한다. append-only `appointment_payment_events`는 처리자·시각·사유를 기록한다. Firestore 결제 필드는 전환 전 백필과 rollback 비교에만 사용하며 새 상태를 이중 쓰지 않는다. 이번 main 저장소 범위는 제한 전이 함수와 사용자 조회·입금자명 제출 계약까지만 준비한다. 관리자 웹과 관리자 서버는 별도 `bodeul-admin-web` 저장소에서 입금 확인·검토·환불 호출 경계를 연결하는 후속 작업으로 진행하고 브라우저가 PostgreSQL이나 Firestore 결제 상태를 직접 수정하지 않는다.
