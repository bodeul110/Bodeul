...
- 목록 화면에서는 매니저 이메일과 전화번호를 마스킹해 기본 노출 범위를 줄였다.
- 15분 동안 활동이 없으면 자동 로그아웃해 장시간 열린 관리자 세션을 줄인다.
...
- 2026-05-05 기준 관리자 웹 목록에는 기본 마스킹이 적용됐고, 유휴 세션 자동 로그아웃도 들어갔다.
- 따라서 `민감 필드 마스킹 정책`은 일부 적용 상태이며, 추후 관리자 상세 화면, 로그 산출물, 내보내기 파일까지 같은 기준을 확장할지 결정하면 된다.
