...
- `docs/implementation-status.md`는 마지막 저손상 이력(`055c37d`)을 기준으로 복구하고, 손상된 최근 상세 기록은 개별 보고서 문서 참조로 정리했다.
...
- 앞으로 새로 깨진 문서가 생기면 `implementation-status.md`에는 요약만 남기고, 상세 배치 기록은 성격별 보고서 문서에 분리하는 기준을 유지한다.
