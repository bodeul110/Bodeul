...
이 문서는 2026-04-29 점검을 2026-05-04 당시 코드로 대조한 역사적 기록이다. 아래 '현재'와 해결 여부는 그 시점 기준이다. 이후 PostgreSQL·관리자 서버·RBAC·App Check 계약이 바뀌었으므로 현행 판단은 [Rules 경계](firebase-rules-validation.md), [관리자 RBAC](../architecture/admin-rbac.md), [인프라 리스크](../architecture/infra-risk-review.md)를 따른다.
...
- 다만 로컬에 남는 민감 데이터를 줄이는 조치는 이미 들어갔다.
  - [AndroidManifest.xml](../../app/src/main/AndroidManifest.xml)에서 `allowBackup="false"`
  - [backup_rules.xml](../../app/src/main/res/xml/backup_rules.xml), [data_extraction_rules.xml](../../app/src/main/res/xml/data_extraction_rules.xml)에서 백업 제외
  - [ServiceLocator.java](../../app/src/main/java/com/example/bodeul/data/ServiceLocator.java)에서 Firestore를 `MemoryCacheSettings`로 고정
- 현재 지속 저장되는 앱 내부 설정은 [PermissionGuidePreferences.java](../../app/src/main/java/com/example/bodeul/ui/auth/PermissionGuidePreferences.java) 수준의 비민감 값이 중심이다.
...
- 예약 연결 탐색, 소셜 중복 이메일 확인, 배정 매니저 프로필 참조는 각각
  ...
  callable로 중계된다.
- 관련 정리는 [firestore-hardening.md](firestore-hardening.md)에 따로 이어서 기록돼 있다.

### 4. 네이버 클라이언트 시크릿이 앱 리소스로 들어간다

상태: `해결`

- [app/build.gradle.kts](../../app/build.gradle.kts)에서 `naver_client_secret` 주입은 제거됐다.
- [BodeulApplication.java](../../app/src/main/java/com/example/bodeul/BodeulApplication.java)에서 네이버 SDK 초기화도 제거됐다.
- [FirebaseAuthRepository.java](../../app/src/main/java/com/example/bodeul/data/firebase/FirebaseAuthRepository.java), [LoginActivity.java](../../app/src/main/java/com/example/bodeul/ui/auth/LoginActivity.java)에서는 네이버 로그인을 비활성화 상태로 처리한다.
...
- `admin-web/src/App.tsx` (당시 메인 저장소 경로)에서 관리자 웹은 이제 `onAuthStateChanged`와 `users/{uid}.role == ADMIN` 검증을 함께 사용한다.
- 로그아웃도 실제 `signOut(auth)`를 호출한다.
- 목록 화면에서는 매니저 이메일과 전화번호를 마스킹해 기본 노출 범위를 줄였다.
- 15분 동안 활동이 없으면 자동 로그아웃해 장시간 열린 관리자 세션을 줄인다.
- [../operations/admin-access-qa-checklist.md](../operations/admin-access-qa-checklist.md)에 검증 절차를 따로 정리했다.

### 7. 매니저 서류 Storage 권한과 파일 검증이 느슨하다

상태: `부분 해결`

- [storage.rules](../../storage.rules)에서 경로는 `manager-documents/{managerUserId}/{documentKey}/{fileName}`로 제한돼 있다.
- 읽기 권한은 `ADMIN` 전체 또는 `본인 MANAGER`로 제한돼 있다.
- 이번 최신화에서 업로드 제약도 추가했다.
  - 허용 문서 키: `license`, `nursingLicense` 중 정확히 1종
  - 허용 MIME: `image/jpeg`, `image/png`, `image/webp`
  - 허용 최대 크기: `10MB`
- `idCard`, `criminalRecord`, `healthCertificate` 신규 업로드는 거부한다. legacy `healthCertificate`는 서버 이관 전용이고 기존 신분증·범죄경력 원본은 보존정책 파기 대상으로만 다룬다.
- [check-manager-document-storage.js](../../tools/firebase/check-manager-document-storage.js)와 정리 흐름으로 메타데이터-실객체 불일치와 고아 파일도 점검 가능하다.

판단:

- 권한 경계는 보수적으로 정리됐다.
- 다만 백신 스캔과 이미지 구조 검증까지는 아직 없다. 매니저 증빙 PDF 신규 제출은 안전한 격리 렌더러가 없어 차단한다.
...
- 최신 기준으로는 [AndroidManifest.xml](../../app/src/main/AndroidManifest.xml)에 위험 권한을 남기지 않고 `INTERNET`만 유지한다.
- [PermissionGuideCatalog.java](../../app/src/main/java/com/example/bodeul/ui/auth/PermissionGuideCatalog.java)는 현재 버전에서 시스템 권한을 미리 요청하지 않는다는 안내만 보여주고, 실제 권한 요청은 수행하지 않는다.
...
- `app/google-services.json` (비공개 로컬 설정), `admin-web/firebase.ts` (당시 메인 저장소 경로)에 있는 Firebase API Key는 일반적으로 `프로젝트 식별자` 성격이 강하고, 그 자체를 비밀로 취급하지 않는다.
...
- 최신 코드 기준 판단은 [aes-scope-assessment.md](aes-scope-assessment.md)에 별도로 정리했다.
