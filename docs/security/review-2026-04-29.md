...
- [admin-web/src/App.tsx](../../admin-web/src/App.tsx)에서 관리자 웹은 이제 `onAuthStateChanged`와 `users/{uid}.role == ADMIN` 검증을 함께 사용한다.
...
- [app/google-services.json](../../app/google-services.json), [admin-web/firebase.ts](../../admin-web/firebase.ts)에 있는 Firebase API Key는 일반적으로 `프로젝트 식별자` 성격이 강하고, 그 자체를 비밀로 취급하지 않는다.
