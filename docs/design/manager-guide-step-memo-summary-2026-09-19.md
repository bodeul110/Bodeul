...
동행 마지막 일지 화면에서 매니저가 앞 단계에서 직접 저장한 메모 원문을 다시 읽을 수 있게 한다. 원문은 AI로 요약하거나 의료적으로 재해석하지 않으며, 최대 300자의 `managerJournal` 입력값으로 복사하거나 덮어쓰지 않는다.
...
- 마지막 일지 제출 또는 리포트 재시도 화면에서만 읽기 전용 목록을 보여준다.
...
Core API는 현재 `CARE_ENDED` 이후 매니저 응답에서 `guardianUpdate`, `locationSummary`, `fieldPhotoNote`, `medicationNote`, `pharmacySummary`를 모두 마스킹한다. 따라서 프론트엔드 카드와 조합 로직은 준비할 수 있지만, 종료 뒤 재진입에서 원문을 실제로 보여주려면 다음 조건을 만족하는 별도 서버 계약이 필요하다.
