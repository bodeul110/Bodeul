...
| --- | ---: | --- | --- | --- |
| 첫 화면 `8:179` | 390×998 | 전용 Activity 없음, `RoleSelectionActivity`가 launcher | 신규 필요 또는 생략 결정 | 단순 브랜드 스플래시인지 행동 가능한 랜딩인지 확인한 뒤 추가한다. 선행 PR #381은 병합됐다. |
...
| ---: | --- | --- | --- | --- |
| 1 | 동행 가이드 1 `8:3242` | `ManagerGuideActivity`의 첫 단계 | 부분 대응 | #382 종료·완료 계약 반영 완료 |
| 2 | 동행가이드2 `8:2950` | 텍스트 전용 동적 단계 화면 | 계약 대기 | [#391](https://github.com/bodeul110/Bodeul/issues/391): 승인 자산·사용 권리·비식별 검수와 미디어 계약 필요 |
| 3 | 동행가이드3 `8:835` | 동적 단계 화면 | 부분 대응 | #382 종료·완료 계약 반영 완료 |
| 4 | 동행가이드4 `8:899` | 동적 단계 화면 | 부분 대응 | #382 종료·완료 계약 반영 완료 |
...
| 13 | 동행가이드 13 `8:1911` | 동적 단계와 리포트 제출 | 부분 대응 | #382 최종 완료 계약 반영 완료 |
...
2026-09-01 확인 결과 메인 저장소의 관련 PR과 후속 Figma PR은 모두 병합됐고 열린 PR은 0개다. 관리자 웹 PR #44만 열려 있으며 Android 화면 구현과 직접 충돌하지 않는다.
...
과거 병합 순서는 #381 → #382 → #380 → #383이었고 현재 `master`에 모두 반영됐다. 이후 화면 구현은 병합 대기 없이 최신 `master`를 기준으로 시작하되, 관리자 웹 역할·민감정보 경계는 열린 PR #44와 production Preview 검증 결과를 별도로 따른다.
...
- `8:179`는 로고와 진행 표시가 있는 브랜드 시작 화면임을 확인했다. 선행 #381은 병합됐으며 전용 Activity와 Manifest 변경은 별도 묶음으로 구현한다.
...
#382와 #380이 병합된 최신 계약을 기준으로 1~13단계를 `stepCode` 단위로 맞춘다. 상봉번호 변형은 제외한다. 가이드 2는 [#391](https://github.com/bodeul110/Bodeul/issues/391)에서 자산 ID·버전·대체문구 계약과 fail-closed 안내를 먼저 구현하고, 실제 재생은 승인 자산·사용 권리·비식별 검수와 서명 재생 경로가 준비된 뒤 연결한다. `canAdvance=false`와 `GUIDE_NOT_READY`에서는 작동하는 것처럼 보이는 CTA를 만들지 않는다.
...
- 리스크: frame 이름만 확인한 두 예약 알림창과 채팅형 시안은 상호작용·제품 범위가 아직 확정되지 않았다. 가이드 2 영상은 자산·사용 권리·비식별 검수와 안전한 미디어 계약이 없으므로 텍스트 설명에 임의 URL을 넣지 않는다.
...
- [#391](https://github.com/bodeul110/Bodeul/issues/391) 가이드 2 영상 자산·권리·비식별 검수와 미디어 계약
- 최신 `master` 기준 화면별 구현 브랜치 생성
- 브랜드 시작 화면과 Manifest 진입 경로 구현
