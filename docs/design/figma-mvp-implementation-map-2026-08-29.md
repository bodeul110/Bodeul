...
#382와 #380이 병합된 최신 계약을 기준으로 1~13단계를 `stepCode` 단위로 맞춘다. 상봉번호 변형은 제외하고, 가이드 2 영상은 [#391](https://github.com/bodeul110/Bodeul/issues/391)에서 승인 자산·사용 권리·비식별 검수와 미디어 계약이 준비된 뒤 구현한다. `canAdvance=false`와 `GUIDE_NOT_READY`에서는 작동하는 것처럼 보이는 CTA를 만들지 않는다.
