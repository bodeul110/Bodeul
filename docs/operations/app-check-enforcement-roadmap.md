...
| Firebase 서비스 | 개발·production Firestore, Storage, Authentication 모두 `UNENFORCED` |
...
| 운영 | `bodeul-prod-110`에 Android/Web 앱과 기본 Auth domain을 등록했다. release SHA-256, Play Integrity, Web provider, Vercel Production 환경과 실제 도메인은 독립 구성하며 개발 debug token과 provider secret을 복사하지 않는다. |
...
| 2. 배포 환경 토큰 검증 | 운영 후보 빌드와 Vercel URL에서 실제 provider 토큰 발급 확인 | Android release 빌드, 관리자 웹 preview/live에서 App Check 실패 없이 로그인/예약/심사 흐름 통과 | 실기기 테스트, Vercel preview/live 테스트, App Check 메트릭 확인 |
