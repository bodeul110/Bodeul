...
- 별도 저장소의 Next.js 관리자 웹은 production 전 reCAPTCHA Enterprise를 적용한다. Vite rollback의 v3 경로는 운영 기준으로 확장하지 않는다.
...
| Next.js 관리자 서버 | Firebase ID token·DB role 검증 완료, App Check custom backend 검증은 아직 없음 | `bodeul-admin-web` |
...
| reCAPTCHA Enterprise | API 활성, App Check site key와 제한된 `SCORE` Web key | provider 기반 설정과 production hostname 제한 완료, 클라이언트 token 증거 없음 |
...
현재 production 단계는 `preparing`이다. Web provider 기반 설정은 완료했지만 Android provider와 클라이언트 token 전송, monitoring, Functions enforcement나 Firebase 서비스 enforcement는 완료한 상태가 아니다.
...
| 운영 | `bodeul-prod-110`에 Android/Web 앱을 등록했고 canonical 관리자 웹 hostname용 reCAPTCHA Enterprise key, Web App Check와 Auth domain을 구성했다. 현재 App Check 단계는 `preparing`이다. release SHA-256, Play Integrity, Vercel Production 환경과 클라이언트 token 전송은 독립 구성하며 개발 debug token과 provider 설정을 복사하지 않는다. |

관리자 웹의 Next.js 전환은 완료했다. Vite reCAPTCHA v3 설정에 추가 투자하지 않고, Next.js에서 reCAPTCHA Enterprise를 연결해 Preview `VALID` 요청을 확인한 뒤 운영 설정으로 승격한다.
...
| 4. custom backend 검증 | Spring Core API와 Next.js 관리자 서버 보호 | Spring의 Android debug `valid` 완료. release Play Integrity와 Next.js provider token 확인 필요 | `X-Firebase-AppCheck` 검증을 observe 후 enforce로 전환 |
...
- 현재 구현은 reCAPTCHA v3 기준이므로 새 Next.js 관리자 웹을 Enterprise provider로 전환한 뒤 production 후보로 본다.
...
현재 production에서는 App Check를 강제하지 않는다. Web은 canonical production hostname으로 제한한 reCAPTCHA Enterprise key와 provider 기반 설정을 완료했지만, Android exact release SHA-256과 Play 연결, Next.js token 전송은 준비되지 않았다. Identity Platform·Firestore·Storage는 모두 기본 `OFF`이고 production callable Functions와 최근 7일 Android·Web `ALLOW`·`VALID` 요청도 0건이므로 단계는 `preparing`이다. 과거 개발 Android debug와 Spring Core API preview `valid` 기록은 개발 경로의 근거일 뿐 production 전환 증거로 재사용하지 않는다. 팀 소유 release key와 Play 연결, Next.js header·서버 검증을 준비한 뒤 `UNENFORCED` 관찰과 실제 요청 증거를 확보한다. 그 다음 Functions, custom backend, Storage, Firestore, Authentication 순서로 제한적으로 강제하고 각 단계를 별도로 rollback 검증한다.
