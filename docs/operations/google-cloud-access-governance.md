...
2026-09-22 조회 기준 공식 조직 소속 `bodeul-shared-billing1`은 `open=true`이며, 두 프로젝트 모두 이 계정에 연결돼 `billingEnabled=true`다. 기존 개인 결제 계정은 닫힌 상태이며 프로젝트 연결이 없다. 이번 조직 이전에서는 결제 연결이나 결제 수단을 변경하지 않았다.
