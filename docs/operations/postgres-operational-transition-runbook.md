...
Firebase 인프라는 유지하면서 운영 DB를 Supabase PostgreSQL로 옮기기 위해 사용자가 만들어야 하는 외부 리소스, GitHub 설정, 초기 검증 순서를 고정한다. Oracle Cloud API 서버는 즉시 필수 리소스가 아니라 PostgreSQL 접근 경계가 필요해지는 시점에 준비한다.
...
DB schema와 import dry-run만 진행하는 동안에는 GitHub Environment secret을 만들지 않는다. DB connection string은 로컬 비공개 설정이나 사용자가 직접 입력하는 GitHub secret으로만 다룬다.
...
2. Supabase 프로젝트 리전과 프로젝트 이름만 Codex에게 알려준다.
3. DB connection string 원문은 채팅, Issue, PR에 적지 않는다.
4. GitHub에는 아직 DB secret을 넣지 않는다. import/API 연결 방식이 PR로 준비된 뒤 넣는다.
