...
Firebase 인프라는 유지하면서 운영 DB를 Supabase PostgreSQL로 옮기기 위해 인프라 담당자가 만들어야 하는 외부 리소스, GitHub 설정, 초기 검증 순서를 고정한다. 2026-07-07 기준 Oracle Cloud API 서버는 #140의 preview 실연동 검증에서 먼저 준비할 수 있지만, production 서버나 live 전환 기준은 아니다.
...
Oracle Cloud VM은 #140의 preview 실연동 검증 범위에서 개발용으로 준비할 수 있다. 이 VM은 #123의 병원 가이드 Firestore/API 응답 비교를 실제 배포 API 응답으로 마무리하기 위한 검증 환경이며, production cutover나 운영 서버 확정이 아니다.
...
- Vercel preview 또는 Firebase Hosting preview 관리자 웹 origin을 `BODEUL_API_ALLOWED_ORIGINS`에 등록할 수 있다.
...
DB schema와 import dry-run만 진행하는 동안에는 GitHub Environment secret을 만들지 않는다. #140의 수동 preview 검증은 Oracle 서버 환경값과 Vercel Environment Variable로 시작할 수 있으며, GitHub Actions 기반 API 배포가 필요해질 때 `api-preview`를 만든다. DB connection string은 로컬 비공개 설정, 서버 비공개 환경값, 또는 담당자가 직접 입력하는 GitHub secret으로만 다룬다.
...
8. 문제가 없으면 #140에서 preview API 응답 JSON을 저장하고 #123에서 Firestore/API 비교를 마무리한다.
...
4. 유지: GitHub에는 자동 배포가 필요해지기 전까지 DB secret을 넣지 않는다. #140 수동 preview 검증은 Oracle/Vercel/Supabase/Firebase 각 플랫폼의 비공개 환경값으로 진행할 수 있다.
5. 진행 가능: #140을 위해 Oracle 개발 VM을 만들 경우 명칭은 `bodeul-dev-api-01`로 둔다. production VM은 #134 이후 별도 결정한다.
