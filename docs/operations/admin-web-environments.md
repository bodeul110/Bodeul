...
- 완료 조건: WIF-only 수동 preview 배포가 성공하면 `ADMIN_WEB_FIREBASE_TOKEN` secret을 삭제한다.
...
7. 성공 후 `ADMIN_WEB_FIREBASE_TOKEN` secret을 삭제한다.
...
1. fallback 제거 후 WIF-only preview workflow를 한 번 수동 실행해 Hosting URL과 Auth authorized domain 동작을 확인한다.
2. GitHub Environment `admin-web-preview`에서 `ADMIN_WEB_FIREBASE_TOKEN` secret을 삭제한다.
