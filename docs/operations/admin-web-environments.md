...
1. 모든 PR마다 preview 배포를 자동 실행할지 재검토한다.
2. `admin-web-production` 운영 Firebase 프로젝트와 Hosting site를 확정한다.
3. production Environment 값을 설정한다.
4. production 배포용 WIF 서비스 계정과 권한 범위를 정한다.
5. App Check 적용 시점과 강제 기준을 production 배포 전에 재확인한다.
6. production live workflow는 required reviewer, protected branch, preview 검증 기록 조건이 준비된 뒤 추가한다.
