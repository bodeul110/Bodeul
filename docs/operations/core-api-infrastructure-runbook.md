...
- 개발 DB의 V1~V5 migration과 세션 백필, RLS, Core/Admin runtime 읽기·배정 권한 검증이 완료됐다.
- 세션 진행·리포트용 V6 최소 컬럼 쓰기 권한은 개발 DB run `29639792606`에서 검증했다. Core API Preview 배포와 무인증 경계 확인을 완료했고 실제 token 역할 검증은 남아 있다.
...
- production Google Cloud/Firebase `bodeul-prod-110`과 Supabase `bodeul-prod`를 생성했다. Artifact Registry, WIF, deploy/runtime 서비스 계정, DB Secret Manager version과 Flyway V1~V3는 준비됐고 Cloud Run 서비스는 첫 승인 배포 전이다.
...
2026-07-18 [Core API Preview Deploy #29639915209](https://github.com/bodeul110/Bodeul/actions/runs/29639915209)로 commit `9d08c1be5c84c28c6c34094e0b5c5511ce02de46`을 배포했다. 리비전 `bodeul-core-api-preview-00010-pd9`가 트래픽 100%를 처리하며 `/health`는 200 `UP`, `/api/companion-sessions`는 무인증 요청에 401 `missing_authorization`을 반환했다. 역할별 200·403과 version 충돌은 실제 Firebase ID token으로 별도 검증한다.
