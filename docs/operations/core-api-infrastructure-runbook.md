...
- 개발 DB의 V1~V6 migration과 세션 백필, RLS, Core/Admin runtime 읽기·배정 권한 검증이 완료됐다.
- 세션 진행·리포트용 V6 최소 컬럼 쓰기 권한은 개발 DB run `29639792606`에서 검증했다. Core API Preview 배포, 무인증 경계와 실제 token 역할 검증을 완료했다.
- 예약 후속 처리용 V7 최소 컬럼 쓰기 권한과 Core API·Android 연결은 구현됐다. PostgreSQL 17 임시 인스턴스에서 V1~V7 연속 적용과 권한·부분 수정·버전 충돌을 검증했으며, 개발 DB 적용과 Preview 역할 검증은 병합 후 migration 단계로 남아 있다.
