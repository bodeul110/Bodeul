...
- production 프로젝트, 서비스, secret은 아직 만들지 않았으며 이름과 리전은 [Production 인프라 기본값](production-infrastructure-defaults.md)에 고정했다.
...
Supabase 개발 DB도 Tokyo이므로 Core API를 Tokyo에 둔다. 초기 사용자 기반이 한국이고 production Supabase도 Tokyo로 만들 예정이므로 production Cloud Run 리전도 `asia-northeast1`로 고정한다.
