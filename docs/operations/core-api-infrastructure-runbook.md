...
- 실제 Firebase ID token 시나리오는 Issue #157에서 계속 검증한다.
...
기존 GitHub Environment의 `CORE_DB_*` secret은 실제 Firebase ID token과 PostgreSQL role 조회가 끝날 때까지만 보존한다. 검증 후 중복 secret을 삭제하고 Secret Manager를 runtime source of truth로 삼는다.
...
배포와 rollback 결과는 `docs/reports/`와 Issue #156에 기록한다. 실제 token 시나리오는 Issue #157에서 이어간다.
