...
- `request-seed-completed`
  - 시나리오 이름: `종료 후속 처리`
  - 의미: 리포트, 후기와 정산 재확인이 남아 있는 완료 요청
  - 확인 포인트: 보호자 리포트, 환자 후기/후속, 관리자 후속 처리
  - 참고: 현재 seed는 SOS 값과 긴급 이슈를 만들지 않는다. 기존 배포·백업에 남은 legacy SOS 값은 읽기 호환 확인용으로만 취급한다.

### 매니저 서류 시나리오

- `manager@bodeul.app` 계정에는 `license` 자격 증빙 이미지 1종이 준비돼 있다.
- `nursingLicense` 시나리오는 별도 fixture로 같은 제출·심사 계약을 확인한다.
- 신분증·범죄경력 원본과 legacy `healthCertificate`는 신규 테스트 데이터로 만들지 않는다.
- 관리자 웹에서 자격 증빙 미리보기와 승인/반려 흐름을 바로 확인할 수 있다.
