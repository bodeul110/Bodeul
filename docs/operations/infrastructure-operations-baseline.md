...
| 관리자 웹 preview | GitHub Actions OIDC/WIF 기반 preview workflow 유지 |
...
| API 경계 | `bodeul-api` 구현 시작 완료. 관리자 웹 병원 가이드 read API 1차 연결 완료, 배포 환경 검증은 후속 |
...
| Dependabot/취약점 | `uuid` 전이 취약점 검토 이슈 #103 open |
...
- live URL 기준은 `https://bodeul-dev.web.app`이다.
...
preview 배포는 `admin-web-preview` GitHub Environment와 Google Cloud Workload Identity Federation을 사용한다. Firebase refresh token fallback은 제거된 상태다.
...
| 항목 | 상태 | 설명 |
| --- | --- | --- |
| API 실행 환경 | 미정 | Oracle VM, Cloud Run, Firebase Hosting Functions rewrite 등 후보 검토 필요 |
| `api-preview` GitHub Environment | 후보 | 실제 배포 PR이 시작될 때 생성 |
...
- 병원 가이드 화면의 Firestore/API 응답 비교 기록은 #123에서 남긴다.
...
- 최근 PR #121 기준 `API Build`, `Admin Web Build`, `Android Preflight`, `CodeQL` 통과
- code scanning open alert 0건
- GitHub Actions Dependabot Updates에서 `/api` uuid 업데이트 실패 run이 있었고, 관련 취약점 검토는 #103에서 추적한다.

정합성 주의:

- #88은 API 골격과 초기 경계 구축 기준으로 완료 처리했다.
- #113은 관리자 웹 1차 read API 연결 기준으로 완료 처리했다.
- #122는 API 환경변수와 CORS origin 확정을 추적한다.
...
| API 서버 | VM/Cloud Run 등 실행 환경 비용과 로그 저장 비용이 생긴다. | 실제 클라이언트 연결 전까지 실행 환경을 만들지 않는다. |
...
| [#74](https://github.com/bodeul110/Bodeul/issues/74) | 관리자 웹 레포 분리 기준 검토 필요 |
| [#103](https://github.com/bodeul110/Bodeul/issues/103) | `uuid` 전이 취약점 업데이트 범위 검토 필요 |
| [#122](https://github.com/bodeul110/Bodeul/issues/122) | 관리자 웹 API 환경변수와 CORS origin 설정 확정 필요 |
| [#123](https://github.com/bodeul110/Bodeul/issues/123) | 병원 가이드 Firestore/API 응답 비교 기록 필요 |
