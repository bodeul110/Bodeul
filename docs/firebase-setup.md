...
현재는 `Cloud Functions`가 두 단계로 동작한다.
...
현재 단계에서는 작업 문서 생성까지만 구현되어 있고, 실제 카카오 알림톡 발송 함수는 다음 단계에서 연결한다.
...
1. 앱에서 예약 생성
2. Firestore에 `appointmentRequests` 저장
3. 매일 오전 9시 `syncAppointmentReminderJobs` 실행
4. 조건에 맞는 요청의 `appointmentReminderJobs` 생성
5. 10분마다 `deliverAppointmentReminderJobs`가 큐를 읽어 재검증 후 발송
6. 관리자 계정은 `dispatchAppointmentReminderJobs` callable로 수동 발송도 가능
