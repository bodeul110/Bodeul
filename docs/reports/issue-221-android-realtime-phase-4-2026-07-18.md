...
- Core API는 PostgreSQL 커밋 뒤 비동기 이벤트로 FCM을 보낸다. 채팅 본문과 좌표 대신 세션·예약 식별자, 발신 역할과 고정 안내만 전달한다.
...
- Core API의 채팅·위치 알림은 transaction commit 이후 별도 executor에서 발송하며 실패해도 업무 쓰기를 되돌리지 않는다.
...
- V12 실제 migration은 아직 실행하지 않았다.
- Cloud Run preview에는 새 Core API revision과 FCM 권한을 아직 적용하지 않았다.
- 개발 Firebase에는 새 Firestore Rules를 아직 배포하지 않았다.
- 실기기 private 구독·재연결·FCM과 Core-only 첨부 제한은 아직 종단 검증 전이다.
- Realtime 동시 연결과 메시지 사용량은 개발 부하 검증 뒤 기록한다.
...
- 개발 DB V12 migration과 Cloud Run preview 배포
- Cloud Run runtime에 Firestore read-only와 FCM 발송 권한 적용
- 개발 Firestore Rules 배포
- 실기기 채팅·읽음·위치·재연결·FCM 종단 검증
- 개발 Realtime 연결 수·메시지 수 부하와 비용 확인
- Core-only 세션의 첨부 업로드·다운로드 권한 이전
- production 전환은 연말 운영 승인 뒤 별도 실행
- #222에서 채팅 180일, 첨부 30일, 위치 24시간 만료 자료를 정리하는 일일 파기 구현

## 리스크

- FCM은 보조 경로이므로 발송 실패가 채팅 저장을 실패시키지 않는다. 재시도와 실패율 관측을 보강해야 한다.
