...
>
> 2026-08-30 확정 계약: 신규 수집은 `license` 또는 `nursingLicense` 자격 증빙 1종, JPEG·PNG·WebP, 10 MiB 이하로 제한한다. 신분증·범죄경력 원본과 신규 `healthCertificate` 쓰기는 허용하지 않는다.
...
#313의 정책 질문은 2026-08-30 최소수집 기준으로 해소됐다. Android, Firestore/Storage Rules, Functions 보존 작업과 운영 도구는 PR #383에서 같은 계약으로 반영하고, 관리자 웹은 별도 저장소 PR #44에서 후속 적용한다. 운영 활성화 전 실제 legacy 객체 이관 dry-run과 파기 증적은 출시 게이트에서 확인한다.
