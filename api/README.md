...
Issue #140 범위에서는 Oracle API 실행 환경, Supabase `DATABASE_URL`, Firebase Admin 인증, Vercel preview 관리자 웹 API 모드를 묶어 실제 배포에 가까운 검증을 진행한다. 이 검증은 production 전환이 아니라 #123의 병원 가이드 Firestore/API 응답 비교를 가능하게 하는 preview 환경 구축이다.
...
| 관리자 웹 | Vercel preview 또는 Firebase Hosting preview에서 `VITE_BODEUL_DATA_BACKEND=api` |
| CORS | `BODEUL_API_ALLOWED_ORIGINS`에 해당 preview origin만 등록 |
