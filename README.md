...
> [!IMPORTANT]
> 현재 Android 앱과 관리자 웹의 주요 운영 데이터는 Firebase를 기본 경로로 사용합니다. PostgreSQL은 전체를 한 번에 교체하는 방식이 아니라 `bodeul-api`를 경유해 도메인별로 검증·전환하고 있습니다.
...
| 관리자 웹 | React 19, TypeScript 6, Vite 8, Tailwind CSS 4 | 서류 심사와 운영 백오피스 |
| API | Node.js 22, TypeScript, Firebase Admin SDK, `pg` | 인증·인가와 PostgreSQL 접근 경계 |
...
| `api/` | `bodeul-api` 서버, Firebase token 검증, PostgreSQL 연결과 관리자 API |
...
`bodeul-api`는 클라이언트가 PostgreSQL에 직접 접속하지 않도록 인증, 인가와 데이터 접근을 통제하는 얇은 서버 경계입니다.
